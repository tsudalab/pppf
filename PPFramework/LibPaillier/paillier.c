...
void
init_rand( gmp_randstate_t rand, paillier_get_rand_t get_rand, int bytes, void *usrPtr)
{
	void* buf;
	mpz_t s;

	buf = malloc(bytes);
	get_rand(buf, bytes, usrPtr);
                                ...
                                void *usrPtr )
{
	...
	*prv = (paillier_prvkey_t*) malloc(sizeof(paillier_prvkey_t));
	...
	init_rand(rand, get_rand, modulusbits / 8 + 1, usrPtr);
	do
	{
		do
                             ...
                             void *usrPtr)
{
	...
	mpz_init(r);
 	init_rand(rand, get_rand, pub->bits / 8 + 1, usrPtr);
	do
...
{
	return paillier_plaintext_from_bytes(str, (int) strlen(str));
}
...
paillier_pubkey_t*
paillier_pubkey_from_hex(const char* str )
{
	paillier_pubkey_t* pub;
...
paillier_prvkey_t*
paillier_prvkey_from_hex(const char* str, paillier_pubkey_t* pub )
{
	paillier_prvkey_t* prv;
...
void
paillier_get_rand_devrandom( void* buf, int len, void *junk)
{
	paillier_get_rand_file(buf, len, "/dev/random");
}

void
paillier_get_rand_devurandom( void* buf, int len, void *junk)
{
